...
class EthrDidResolver {
    ...
    async changeLog(identity, networkId, blockTag = 'latest') {
        ...
        while (previousChange) {
            ...
            if (logs.length === 0) {
                throw new Error(`No logs found for block ${blockNumber} but previousChange points here. ` +
                    `The RPC node may not have historical log data. Use an archive node for complete DID resolution.`);
            }
            const { events, previousChange: pc } = (0, logParser_js_1.logDecoder)(contract, logs, blockNumber);
                ...
                if (event._eventName === helpers_js_1.eventNames.DIDDelegateChanged) {
                    ...
                    if (match) {
                        ...
                        switch (section) {
                            ...
                            case 'svc': {
                                serviceCount++;
                                let encodedService = null;
                                try {
                                    encodedService = (0, ethers_1.toUtf8String)(currentEvent.value);
                                }
                                catch {
    ...
    _unused, options) {
        ...
        try {
            ...
            const isArchiveError = message.includes('missing trie node') ||
            ...
            const isConnectivityError = message.includes('could not detect network') ||
                message.includes('timeout') ||
                e?.code === 'NETWORK_ERROR' ||
                e?.code === 'TIMEOUT';
            // Server responded but with an error — may indicate missing historical data on non-archive nodes
            const isServerError = message.includes('missing response') || message.includes('SERVER_ERROR') || e?.code === 'SERVER_ERROR';
